...
export const Marker = ({idx, tag, position, address, title, description, telno, onFire, setPos, mapClicked, visible, setSelectedIdx}: MarkerProps) => {
    ...
    return (
        <CustomOverlayMap position={position} onCreate={removeZindex}>
            <button id={`tagmarker${idx}`} className={`grid w-6 h-6 ${onFire ? tagOrderBgGradientColor[tagToOrder[tag]]: tagOrderBgColor[tagToOrder[tag]].normal} place-content-center rounded-[3px] text-white opacity-90 z-10`}
                onClick={() => {
